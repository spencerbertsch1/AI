...
Once that occurs, then the algorithm "backtracks" and undoes the decision that caused the problem and tries another value from the variable's domain, then continues on in this
...
The minimum remaining value (MRV) heuristic is a technique that orders the values in the domain before choosing and returning the first. It's used in the *select_unassigned_variable* method in the CSP. 
...
The MRV heuristic returns the variable that has the fewest possible remaining values in the constraint graph. For example if we could pick var1 that has 
...
achieved better or consistent performance for the map-coloring problem when the (max/min) functions were reversed. When the MRV returned the variable with the maximum
...
A solution to the circuits problen was found after visiting 19 nodes. See the below tables for an overview
...
The map-coloring problem's performance is improved when inference is turned on, and the performance is improved even further when heuristics are turned on as well. 
